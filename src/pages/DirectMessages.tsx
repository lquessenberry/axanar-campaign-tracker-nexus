...
import { LCARSListItem } from "@/components/lcars";
...
import { useOptimizedMessages } from "@/hooks/useMessages";
  ...
  useEffect(() => {
    if (selectedConversationId) {
      ...
      const unreadMessages = conversationMessages
        .filter((msg) => !msg.is_read && msg.sender_id !== effectiveUserId)
        .map((msg) => msg.id);
  ...
  const handleSendMessage = async (recipientId: string, content: string) => {
    try {
      ...
      await sendMessage(
        recipientId,
        content,
        activeTab === "support" ? "support" : "direct",
      );
      setMessageInput("");
  ...
  const selectedConversation = useMemo(
    () =>
      filteredConversations.find(
        (c) => c.partner_id === selectedConversationId,
      ),
  ...
  return (
    <>
      ...
      <PullToRefresh onRefresh={handleRefresh}>
        <div className="h-screen md:h-[calc(100vh-113px)] bg-background flex overflow-hidden">
          ...
          <AnimatePresence>
            {sidebarOpen && (
              ...
              >
                ...
                {/* Conversations List */}
                <div
                  data-tour="dm-conversations"
                  className="px-4 md:px-6 flex-1 overflow-hidden flex flex-col"
                >
                    ...
                    {loading ? (
                      ...
                      filteredConversations.map((conv) => (
                        <LCARSListItem
                          ...
                          onClick={() =>
                            handleSelectConversation(conv.partner_id)
                          }
                          accentColor={
                            activeTab === "support" ? "secondary" : "primary"
                          }
                        >
                    ...
                    <div className="flex items-center gap-2 md:gap-3 flex-1 min-w-0">
                      ...
                      <div className="min-w-0 flex-1">
                        <h2 className="font-semibold text-xs md:text-sm truncate">
                          {selectedConversation.partner_full_name ||
              ...
              <div className="mt-auto">
                ...
                <div className="border-t border-border bg-card/40 backdrop-blur-3xl px-3 md:px-6 py-3 md:py-6 pb-safe">
                  <p className="text-[10px] md:text-xs text-muted-foreground text-center hidden md:block">
                    Press{" "}
                    <kbd className="px-2 py-0.5 bg-muted rounded text-[10px]">
                      ⌘K
                    </kbd>{" "}
                    to focus •
                    <kbd className="px-2 py-0.5 bg-muted rounded text-[10px] ml-1">
                      ⌘B
                    </kbd>{" "}
                    to toggle sidebar •
                    <kbd className="px-2 py-0.5 bg-muted rounded text-[10px] ml-1">
            ...
            {isAdmin && selectedConversationId && activeTab === "support" && (
              ...
              >
                <UserSupportContext
                  ...
                  onOpenDonorSearch={() =>
                    toast.info("Opening Donor Account Link Tool...")
                  }
                  onOpenPledgeRestore={() =>
                    toast.info("Opening Pledge Data Restoration...")
                  }
                  onOpenAccountMerge={() =>
                    toast.info("Opening Account Merge Tool...")
                  }
                />
              </motion.div>
            )}
