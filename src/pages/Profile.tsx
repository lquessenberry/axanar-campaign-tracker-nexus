...
import {
  ...
  useAdminUserProfile,
} from "@/hooks/useAdminUserProfile";
...
const Profile = () => {
  ...
  const { threads: forumThreads, comments: forumComments } =
  ...
  const { data: adminUserData, isLoading: adminProfileLoading } =
    useAdminUserProfile(shouldFetchOtherProfile ? targetUserId : "");
  ...
  const isLoading = isViewingOtherUser
    ? adminProfileLoading
    : ownProfileLoading;
  const updateProfile = isViewingOtherUser
    ? updateAdminProfile
    : updateOwnProfile;
  ...
  const canRecruit = Boolean(
    totalPledged >= 100 && profile?.full_name && profile?.bio,
  );
          ...
          <Card className="bg-card/50 backdrop-blur-sm border-border/50">
            <CardContent className="p-6">
              ...
              <p className="text-muted-foreground leading-relaxed mb-6">
                {profile?.bio ||
                  'No bio added yet. Click "Settings" to add information about yourself.'}
              </p>
            ...
            <div className="relative overflow-hidden">
              ...
              <div className="relative z-10 container mx-auto px-4 sm:px-6 lg:px-8 py-8 max-w-7xl">
                <IdentityPanel
                  ...
                  onEdit={() => {
                    console.log(
                      "📝 IdentityPanel: Edit Profile clicked, switching to settings section",
                    );
                  ...
                  onAvatarClick={() => {
                    ...
                    if (
                      avatarInput &&
                      avatarInput.className.includes("hidden")
                    ) {
                      avatarInput.click();
                    }
                  }}
                  onBackgroundClick={() => {
                    ...
                    const backgroundInput = Array.from(bgInputs).find(
                      (input) =>
                        input !==
              ...
              <div data-tour="profile-header">
                <ProfileHeader
                  ...
                  onEdit={() => {
                    console.log(
                      "📝 ProfileHeader: Edit Profile Info clicked, switching to settings section",
                    );
                    setActiveSection("settings");
                    setIsEditing(true);
                  }}
