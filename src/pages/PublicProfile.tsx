...
const PublicProfile = () => {
  ...
  const { data: profileData, isLoading, error } = useVanityProfile(username!);
  ...
  const yearsSupporting =
    pledges?.length && pledges[pledges.length - 1]?.created_at
      ? Math.max(
          1,
          new Date().getFullYear() -
            new Date(pledges[pledges.length - 1].created_at).getFullYear(),
        )
      : 0;

  // Calculate campaigns supported
  const campaignsSupported = [
    ...new Set(pledges?.map((p) => p.campaigns?.name)),
