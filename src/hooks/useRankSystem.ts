...
export interface XPBreakdown {
  ...
  dual_path_bonus: number;
  title_bonus_xp: number;
  active_title_multiplier: number;
  total_posts: number;
...
// Military rank thresholds (canonical — used by RankPips and ProgressSection)
export const MILITARY_RANK_THRESHOLDS: MilitaryRank[] = [
  {
    name: "Fleet Admiral",
    pips: 7,
    pipColor: "bg-yellow-500",
...
/** Look up the military rank for a given XP total */
export const getRankForXP = (xp: number, isAdmin = false): MilitaryRank => {
  if (isAdmin) return MILITARY_RANK_THRESHOLDS[0]; // Fleet Admiral
  return (
    MILITARY_RANK_THRESHOLDS.find((r) => xp >= r.minXP) ||
    MILITARY_RANK_THRESHOLDS[MILITARY_RANK_THRESHOLDS.length - 1]
  );
};
  ...
  return useQuery<RankSystemData>({
    ...
    queryFn: async (): Promise<RankSystemData> => {
      ...
      if (
        !userId &&
        currentUserEmail &&
        isPlatformTeamEmail(currentUserEmail)
      ) {
        isPlatformTeam = true;
        isAdmin = true;
      } else {
        ...
        const { data: adminData } = await supabase
          ...
          .single();
        isAdmin = !!(
          adminData?.is_super_admin || adminData?.is_content_manager
        );
      }
      ...
      const xpBreakdown: XPBreakdown = {
        ...
        dual_path_bonus: profile.dual_path_bonus || 0,
        title_bonus_xp: profile.title_bonus_xp || 0,
        active_title_multiplier: profile.active_title_multiplier || 1.0,
      ...
      // DB now stores unified_xp = MAX(donation, participation) + dual-path bonus
      // Use the DB value as the authoritative total
      const totalXP = xpBreakdown.total;

      // Platform Team and Admins get Fleet Admiral rank automatically
      const militaryRank = getRankForXP(totalXP, isAdmin || isPlatformTeam);
      ...
      if (isAdmin || isPlatformTeam) {
        ...
        progressToNext =
          militaryRank.maxXP > militaryRank.minXP
            ? ((totalXP - militaryRank.minXP) /
                (militaryRank.maxXP - militaryRank.minXP)) *
              100
            : 100;
