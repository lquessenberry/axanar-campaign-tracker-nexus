...
const ContributionHistory: React.FC<ContributionHistoryProps> = ({
  ...
  const handlePledgeClick = (pledge: Pledge) => {
    if (pledge.campaigns) {
      setSelectedCampaign({
        campaign_name: pledge.campaigns.title,
        platform: pledge.campaigns.provider || 'Unknown',
        pledge_amount: pledge.amount,
        contribution_date: pledge.created_at,
        campaign_url: pledge.campaigns.web_url
      });
      setModalOpen(true);
    }
  };
          ...
          {isLoading ? (
            ...
            <div className="space-y-4">
              {pledges.map((pledge) => (
                <div 
                  key={pledge.id} 
                  className="flex gap-4 p-4 border rounded-lg hover:border-primary/50 transition-colors cursor-pointer"
