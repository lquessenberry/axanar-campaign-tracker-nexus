...
const getMilitaryRank = (xp: number) => {
  ...
  return MILITARY_RANK_THRESHOLDS[0];
};
  ...
  <div className="relative flex items-center gap-4 p-4 rounded-lg border bg-card hover:bg-muted/50 transition-colors">
    {!entry.is_account_linked && (
      <div className="absolute inset-0 backdrop-blur-md bg-background/40 rounded-lg z-10 flex items-center justify-between gap-4 px-6">
        ...
        <Link to="/auth?flow=lookup" className="flex-shrink-0">
          <Button variant="default" size="sm" className="gap-2 bg-yellow-600 hover:bg-yellow-700 text-white whitespace-nowrap">
    ...
    <div className="flex items-center gap-3 flex-1 min-w-0">
      <div className="relative">
        <Avatar className="h-10 w-10">
          ...
          <AvatarFallback>{entry.full_name ? entry.full_name.split(' ').map(n => n[0]).join('').toUpperCase() : '?'}</AvatarFallback>
        </Avatar>
        {entry.is_online && <div className="absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full bg-green-500 border-2 border-background" />}
      </div>
      <div className="min-w-0 flex-1">
        {entry.profile_id && (entry.is_online || isRecentlyOnline(entry.last_seen)) ? (
          <Link to={`/profile/${entry.profile_id}`} className="font-medium truncate hover:text-axanar-teal transition-colors inline-block">{entry.full_name}</Link>
        ) : (
        ...
        <div className="flex items-center gap-2 text-xs text-muted-foreground flex-wrap">
          {category === 'unified_xp' && (() => {
            ...
            return <Badge variant="secondary" className={`text-xs ${militaryRank.pipColor.replace('bg-', 'text-')}`}>{militaryRank.name}</Badge>;
          })()}
    ...
    <div className="text-right">
      <div className="flex items-center justify-end gap-2">
        <div className="text-lg font-bold text-axanar-teal">{config.formatValue(entry.metric_value, entry)}</div>
      </div>
      <div className="text-xs text-muted-foreground">
        ${Number(entry.total_donated || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} donated
        <div className="text-xs mt-0.5">{entry.unified_xp.toLocaleString()} ARES</div>
      </div>
    </div>
    <Button size="sm" variant="ghost" onClick={() => onShare(entry)} className="opacity-60 hover:opacity-100">
      <Share2 className="h-4 w-4" />
    </Button>
  </div>
);

const LeaderboardTable: React.FC<LeaderboardTableProps> = ({ data, category, userPosition, onCategoryChange }) => {
  ...
  const groupedByTier = React.useMemo(() => {
    ...
    const groups = { live_now: [] as LeaderboardEntry[], hot: [] as LeaderboardEntry[], daily: [] as LeaderboardEntry[], pillar: [] as LeaderboardEntry[] };
    data.forEach(entry => { const tier = entry.tier || 'pillar'; groups[tier].push(entry); });
    return groups;
  }, [data, category]);
  ...
  const handleShare = (entry: LeaderboardEntry) => {
    ...
    if (navigator.share) { navigator.share({ title: 'Axanar Supporter', text: shareText, url: window.location.origin }); }
    else { navigator.clipboard.writeText(shareText); toast.success("Copied!"); }
  };
    ...
    <Card className="w-full">
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle className="flex items-center gap-2"><Icon className="h-5 w-5 text-axanar-teal" />{config.title}</CardTitle>
          <div className="flex gap-2 flex-wrap">
            {categories.map(({ key, label }) => (
              <Button key={key} variant={category === key ? "default" : "outline"} size="sm" onClick={() => onCategoryChange(key)} className={category === key ? "bg-axanar-teal" : ""}>{label}</Button>
            ))}
          </div>
        </div>
          ...
          {category === 'online_activity' && groupedByTier ? (
            <>
              {groupedByTier.live_now.length > 0 && (
                <div className="space-y-3">
                  <div className="flex items-center gap-2 px-2 py-1"><TierIndicator tier="live_now" /><span className="text-sm text-muted-foreground">{groupedByTier.live_now.length} online</span></div>
                  {groupedByTier.live_now.map((entry) => <EntryCard key={entry.donor_id} entry={entry} config={config} category={category} onShare={handleShare} getRankIcon={getRankIcon} isRecentlyOnline={isRecentlyOnline} getMilitaryRank={getMilitaryRank} />)}
                </div>
              )}
              {groupedByTier.hot.length > 0 && (
                <div className="space-y-3">
                  <div className="flex items-center gap-2 px-2 py-1"><TierIndicator tier="hot" /><span className="text-sm text-muted-foreground">{groupedByTier.hot.length} recent</span></div>
                  {groupedByTier.hot.map((entry) => <EntryCard key={entry.donor_id} entry={entry} config={config} category={category} onShare={handleShare} getRankIcon={getRankIcon} isRecentlyOnline={isRecentlyOnline} getMilitaryRank={getMilitaryRank} />)}
                </div>
              )}
              {groupedByTier.daily.length > 0 && (
                <div className="space-y-3">
                  <div className="flex items-center gap-2 px-2 py-1"><TierIndicator tier="daily" /><span className="text-sm text-muted-foreground">{groupedByTier.daily.length} daily</span></div>
                  {groupedByTier.daily.map((entry) => <EntryCard key={entry.donor_id} entry={entry} config={config} category={category} onShare={handleShare} getRankIcon={getRankIcon} isRecentlyOnline={isRecentlyOnline} getMilitaryRank={getMilitaryRank} />)}
                </div>
              )}
              {groupedByTier.pillar.length > 0 && (
                <div className="space-y-3">
                  <div className="flex items-center gap-2 px-2 py-1"><TierIndicator tier="pillar" /><span className="text-sm text-muted-foreground">{groupedByTier.pillar.length} pillars</span></div>
                  {groupedByTier.pillar.map((entry) => <EntryCard key={entry.donor_id} entry={entry} config={config} category={category} onShare={handleShare} getRankIcon={getRankIcon} isRecentlyOnline={isRecentlyOnline} getMilitaryRank={getMilitaryRank} />)}
                </div>
              )}
            </>
          ) : (
            data.map((entry) => <EntryCard key={entry.donor_id} entry={entry} config={config} category={category} onShare={handleShare} getRankIcon={getRankIcon} isRecentlyOnline={isRecentlyOnline} getMilitaryRank={getMilitaryRank} />)
          )}
        </div>
      </CardContent>
    </Card>
