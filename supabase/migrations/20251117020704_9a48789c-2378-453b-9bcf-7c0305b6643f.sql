-- Fix ambiguous column references in forum activity leaderboard
...
BEGIN
  RETURN QUERY
  SELECT
    ROW_NUMBER() OVER (ORDER BY (COALESCE(threads.thread_count, 0) + COALESCE(comments.comment_count, 0)) DESC, p.created_at ASC)::BIGINT as rank,
    d.id as donor_id,
    ...
    (COALESCE(threads.thread_count, 0) + COALESCE(comments.comment_count, 0))::BIGINT as metric_value,
    COALESCE(pledges.pledge_total, 0) as total_donated,
    COALESCE(EXTRACT(YEAR FROM AGE(CURRENT_DATE, pledges.first_pledge_date))::INTEGER, 0) as years_supporting,
  ...
  WHERE d.auth_user_id IS NOT NULL
    AND (COALESCE(threads.thread_count, 0) > 0 OR COALESCE(comments.comment_count, 0) > 0)
  ORDER BY (COALESCE(threads.thread_count, 0) + COALESCE(comments.comment_count, 0)) DESC, p.created_at ASC
