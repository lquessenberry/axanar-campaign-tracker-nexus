-- Drop and recreate get_leaderboard function to support unified_xp
...
BEGIN
  IF category_type = 'unified_xp' THEN
    ...
    SELECT 
      ...
      COALESCE(p.donor_name, p.full_name, 'Anonymous') as donor_name,
      p.avatar_url,
...
-- Drop and recreate get_user_leaderboard_position function to support unified_xp
...
BEGIN
  IF category_type = 'unified_xp' THEN
    ...
    WITH ranked_users AS (
      SELECT 
        p.id,
        COALESCE(p.unified_xp, 0) as value,
        ROW_NUMBER() OVER (ORDER BY COALESCE(p.unified_xp, 0) DESC) as rank
      FROM profiles p
      WHERE p.unified_xp IS NOT NULL AND p.unified_xp > 0
    ),
    ...
    SELECT 
      ru.rank as user_rank,
      tc.total as total_contributors,
      ru.value as metric_value,
      ROUND((ru.rank::numeric / NULLIF(tc.total, 0)) * 100, 2) as percentile
